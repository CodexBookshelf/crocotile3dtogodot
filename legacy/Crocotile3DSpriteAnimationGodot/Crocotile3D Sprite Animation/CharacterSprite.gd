...
onready var animationSprite = $AnimatedSprite3D
...
func tryState( state ) -> bool:
	var okay = false
	
	# animationPlayer.current_animation will be '' when an animation finished (if not looping)
	# $AnimatedSprite3D.animation will keep animation name when finished
	
	match(state):
		'idle':
			if( animationPlayer.current_animation != 'idle' &&
				animationPlayer.current_animation != 'jump' ): okay = true
		'walk':
			if( animationPlayer.current_animation != 'walk' &&
				animationPlayer.current_animation != 'jump' ): okay = true
		'fall':
			if( animationSprite.animation != 'fall' ): okay = true
		'jump':
			okay = true
...
func getInput(delta):
	...
	animationSprite.flip_h = mirrored
