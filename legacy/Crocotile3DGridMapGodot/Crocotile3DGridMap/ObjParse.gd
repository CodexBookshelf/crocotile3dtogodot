...
# modified by Alex (loads .obj with vertex colors and unlit shading, and crisp texture)
	...
	for line in lines:
		...
		match parts[0]:
			...
			"newmtl":
				...
				currentMat.flags_unshaded = true
				currentMat.vertex_color_use_as_albedo = true
				currentMat.vertex_color_is_srgb = true
				currentMat.params_use_alpha_scissor = true
			...
			_:
				if parts[0] in ["map_Kd","map_Ks","map_Ka"]:
					...
					if textures.has(path):
						...
						currentMat.albedo_texture.set_flags(2)
	return mats
		...
		match parts[0]:
			...
			"v":
				...
				if( parts.size() > 4 ):
					...
					var n_vc = Color(float(parts[4]), float(parts[5]), float(parts[6]))
					vertexColors.append(n_vc)
			"vn":
				# Normal
	...
	for matgroup in faces.keys():
		...
		for face in faces[matgroup]:
			if (face["v"].size() == 3):
				...
				# Vertex Colors
				var fan_vc = PoolColorArray()
				if hasVertexColors:
					if face["v"].size()>0:
						fan_vc.append(vertexColors[face["v"][0]])
						fan_vc.append(vertexColors[face["v"][2]])
						fan_vc.append(vertexColors[face["v"][1]])
				...
				st.add_triangle_fan(fan_v, fan_vt, fan_vc, PoolVector2Array(), fan_vn, [])
		mesh = st.commit(mesh)
